...
###########################################################
# country to ISO
my $fileConvert="countryToISO.txt";
open(IN,$fileConvert);
my %converter=();
while(<IN>)
{
...
#die();
parallel_align($fileTosplit,"tmpOfile.txt",$N,$refile);
...
sub metadataToLists
{
	...
	{
		warn ("Could not find the required column $MV in your metadata file\nPlease check carefully\nExecution will halt now due to missing data\n") if $lock{$MV} eq "na";
	}
	...
	#print "$Iv $Ir $Id $Is $Ip\n";
	#die();
	my $c=0;
	#open(OUT,">tmpTable.csv");
	#print OUT "name\tddate\tddeltas\tsdate\tsdeltas\tcontinent\tarea\tcountry\tregion\tlineade\n";
	...
	while (<IN>)
	{
		...
		print  OUT "$id\t$d\t$delta\t$s\t$delta_sub\t$continent\t$area\t$country\t$region\t$p\n";
	}
	if (-e $ofile)
	{
	...
	for (my $i=0;$i<$numP;$i++)
		...
		push(@outfiles,"$file\_variants");
                unless (defined($pid))
                {
                        ...
                        exec("perl scripts/align.pl --infile $file --ref $ref --outfile $file\_variants") || die "can't exec $!";
                        exit(0);
                }else {
                        push(@childs,$pid);
                }
	...
	if ($arguments{"ref"} eq "na" || (! -e $arguments{"--ref"}))
        {
                print_help();
                my $f=$arguments{"--ref"};
                die("Reason: $f\nNo valid reference sequence file provided. Please provide one!");
        }
...
sub print_help
{
	...
	print " genome sequences; 3) identify sequences that have already been\n";
	...
	print " with respect to the reference genome. This process is executed\n";
	...
	print "                      \t novel data/metadata will be appended to the bottom of the file.\n\n";
        print "Mandatory parameters are --seq, --metadata  and --outfile --ref\n";
        print "the files needs to be in the current folder.\n\n";
        print "\n##EXAMPLE:\n\n";
        print "1# input is metadata.tsv:\nperl addToTable.pl --metadata metadata.tsv --seq sequences.fasta --ref reference.fa --outfile HaploCoV.tsv\n\n";
}
