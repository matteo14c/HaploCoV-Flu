...
$fss=13468;
%conf=(
"genetic"=>"scripts/genetic_code",
);

#PARAMETERS
%arguments=
(
"--in"=>"Filename",                 #F==FALSE, --multi <file> used to pass a multifasta input file
"--genome"=>"Filename2",
"--annot"=>"Filename3",
#####OUTPUT file#############################################
...
$table=$arguments{"--annot"};#"simple_annot_mirror";#"annot_table.pl";#sl5     29728   29768   reg
die("$table need detailed annotation file  in the current folder") unless -e $table;
open(IN,$table);
while(<IN>)
{
	chomp();
...
$header=join(/\t/,$header);
print OUT "POS\tREF\tALT\tannot\t@header\n";
while(<IN>)
{
	($change,@pos)=(split());
	next unless $change=~/\|/;
	...
	print OUT "$pos\t$ref\t$alt\t$annot_string\t@pos\n" if $annot_string=~/missense/;
}
############################################################################################################################
...
sub annot_CDS
{
	...
	if (length($ref)==1 && $ref ne "."&& $alt ne ".")
	{
		...
		return("$namegene:c.$pos_inG$ref>$alt,p.$A1$rel_pos$A2,$eff;");
	}else{
		$eff="";
		...
		if ($NSTOP_R>$NSTOP_G && ($ref=~/\./ || $alt=~/\./))
		{
			...
			return("$namegene:c.$pos_inG$ref>$alt,p.$ref_Seq$truncation*,$eff;");
		...
		#	print "2\n";
			...
			while (length($Cref) %3!=0)
			{
				...
				if ($post>length($gene))
				{
					...
					return("$namegene:c.$pos_inG$ref>$alt,p.$Tref$rel_pos$Talt,$eff;");
				}
			}
			if ($alt=~/\./ || $ref=~/\./)
			{
...
sub print_help
{
        print "This program performs functional annotation of viral genetic variants. It can process only files created by the align.pl utility\n"; 
